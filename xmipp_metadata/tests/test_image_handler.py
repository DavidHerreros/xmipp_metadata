...
projections, angles = ih.generateProjections(100)
ImageHandler().write(projections, os.path.join("test_outputs", "projections.stk"))
md = XmippMetaData(os.path.join("test_outputs", "projections.stk"), angles=angles)
md.setMetaDataColumns(np.ones(len(md)), "subtomo_labels")
md.write(os.path.join("test_outputs", "projections.xmd"), updateImagePaths=True)
